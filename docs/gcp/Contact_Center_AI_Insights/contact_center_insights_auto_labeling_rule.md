...
| `active` | Whether the rule is active. | true | true | The active status of the rule determines whether it is applied to conversations, which can have security implications. | True |  |
...
| `label_key_type` | The type of the label key. Possible values are: `LABEL_KEY_TYPE_UNSPECIFIED`, `LABEL_KEY_TYPE_CUSTOM`. | true | true | The type of the label key can have security implications, especially if it's a custom type. | None |  |
| `label_key` | The label key. | true | true | The label key itself can have security implications, especially if it contains sensitive information. | None |  |
...
| Argument | Description | Required | Security Impact | Rationale | Compliant | Non-Compliant |
|----------|-------------|----------|-----------------|-----------|-----------|---------------|
| `condition` | A optional CEL expression to be evaluated as a boolean value. Once evaluated as true, then we will proceed with the value evaluation. An empty condition will be auto evaluated as true. | false | false | Defines CEL evaluation logic for auto-labeling. No direct security impact. | None |  |
| `value` | CEL expression to be evaluated as the value. | false | false | Defines the evaluated labeling value only. No direct security impact. | None |  |
