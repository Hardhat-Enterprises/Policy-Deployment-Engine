...
| `active` | If true, apply this rule to conversations. Otherwise, this rule is inactive and saved as a draft. | true | false | Disabling this rule prevents conversations from being analyzed, which may reduce policy enforcement and monitoring coverage. But, This attribute does not currently have a policy implementation in this resource. Therefore, security_impact is marked as false. | None | None |
...
| `run_sentiment_annotator` | Whether to run the sentiment annotator. | true | false | Controls sentiment analysis behaviour only. No direct security impact. | None | None |
...
| `run_summarization_annotator` | Whether to run the summarization annotator. | true | false | Controls summarization behaviour only. No direct security impact. | None | None |
