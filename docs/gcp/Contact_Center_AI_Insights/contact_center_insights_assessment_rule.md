...
|----------|-------------|----------|-----------------|-----------|-----------|---------------|
| `location` | Location of the resource. | true | false | The location of the resource is critical for compliance and security purposes. This attribute is already covered by an implemented policy in another relevant resource. Therefore, a separate policy was not created for this resource. | australia-southeast1 |  |
| `active` | If true, apply this rule to conversations. Otherwise, this rule is inactive. | true | false | The active status of the rule determines whether it is applied to conversations, which can have security implications. However, this attribute does not currently have a policy implementation in this resource. Therefore, security_impact is marked as false. | True |  |
