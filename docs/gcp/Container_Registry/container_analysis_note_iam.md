## 🛡️ Policy Deployment Engine: `container_analysis_note_iam`

This section provides a concise policy evaluation for the `container_analysis_note_iam` resource in GCP.

Reference: [Terraform Registry – container_analysis_note_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/container_analysis_note_iam)
...
| `role` | `google_container_analysis_note_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
