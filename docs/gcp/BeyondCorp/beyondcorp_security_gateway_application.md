...
### `security_gateway_id`
- Description: (Required) ID of the Security Gateway resource this belongs to.
- Required: 
- Policy Condition?: 
- Decision / Rationale: 

### `application_id`
- Description: (Required) User-settable Application resource ID. * Must start with a letter. * Must contain between 4-63 characters from `/a-z-/`. * Must end with a number or letter.
- Required: 
- Policy Condition?: 
- Decision / Rationale: 
