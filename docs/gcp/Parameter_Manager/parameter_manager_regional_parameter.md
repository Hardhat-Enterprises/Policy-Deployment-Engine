...
### `location`
- Description: (Required) The location of the regional parameter. eg us-central1
- Required: 
- Policy Condition?: 
- Decision / Rationale: 
...
### `format`
- Description: (Optional) The format type of the regional parameter. Default value is `UNFORMATTED`. Possible values are: `UNFORMATTED`, `YAML`, `JSON`.
- Required: 
- Policy Condition?: 
- Decision / Rationale: 

### `kms_key`
- Description: (Optional) The resource name of the Cloud KMS CryptoKey used to encrypt regional parameter version payload. Format `projects/{{project}}/locations/{{location}}/keyRings/{{key_ring}}/cryptoKeys/{{crypto_key}}`
- Required: 
- Policy Condition?: 
- Decision / Rationale: 
