...
|----------|------------|-----------|----------------|-----------|
| `machine_type` | To create a machine with a [custom type](https://cloud.google.com/dataproc/docs/concepts/compute/custom-machine-types) (such as extended memory), format the value like `custom-VCPUS-MEM_IN_MB` like `custom-6-20480` for 6 vCPU and 20GB of RAM. - - - | none | None | None |
...
| `region` | If region is not provided, the provider region is used. | none | None | None |
| `resource_manager_tags` | * `resource_policies` (Optional) -- A list of self_links of resource policies to attach to the instance. Modifying this list will cause the instance to recreate. Currently a max of 1 resource policy is supported. | none | None | None |
| `tags` |  | none | None | None |
...
| `disk_type` | `"pd-balanced"` or `"pd-standard"`. | none | None | None |
