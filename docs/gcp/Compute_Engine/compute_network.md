...
- Description: (Optional) The BGP best selection algorithm to be employed. MODE can be LEGACY or STANDARD. Possible values are: `LEGACY`, `STANDARD`.
- Required: 
- Policy Condition?: 
- Decision / Rationale: 

### `bgp_always_compare_med`
- Description: (Optional) Enables/disables the comparison of MED across routes with different Neighbor ASNs. This value can only be set if the --bgp-best-path-selection-mode is STANDARD
- Required: 
- Policy Condition?: 
- Decision / Rationale: 

### `bgp_inter_region_cost`
- Description: (Optional) Choice of the behavior of inter-regional cost and MED in the BPS algorithm. Possible values are: `DEFAULT`, `ADD_COST_TO_MED`.
- Required: 
- Policy Condition?: 
- Decision / Rationale: 
...
- Description: immediately after network creation. Defaults to `false`. <a name="nested_params"></a>The `params` block supports:
- Required: 
- Policy Condition?: 
- Decision / Rationale: 

### `resource_manager_tags`
- Description: (Optional) Resource manager tags to be bound to the network. Tag keys and values have the same definition as resource manager tags. Keys must be in the format tagKeys/{tag_key_id}, and values are in the format tagValues/456.
- Required: 
- Policy Condition?: 
- Decision / Rationale: 
