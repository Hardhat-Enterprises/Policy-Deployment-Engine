## 🛡️ Policy Deployment Engine: `compute_backend_service_iam`

This section provides a concise policy evaluation for the `compute_backend_service_iam` resource in GCP.

Reference: [Terraform Registry – compute_backend_service_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/compute_backend_service_iam)
...
| `role` | `google_compute_backend_service_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
