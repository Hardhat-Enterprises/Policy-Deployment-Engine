## 🛡️ Policy Deployment Engine: `compute_instant_snapshot_iam`

This section provides a concise policy evaluation for the `compute_instant_snapshot_iam` resource in GCP.

Reference: [Terraform Registry – compute_instant_snapshot_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/compute_instant_snapshot_iam)
...
|----------|------------|-----------|----------------|-----------|
| `zone` | the value will be parsed from the identifier of the parent resource. If no zone is provided in the parent identifier and no zone is specified, it is taken from the provider configuration. | none | None | None |
| `name` |  | none | None | None |
...
| `role` | `google_compute_instant_snapshot_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
