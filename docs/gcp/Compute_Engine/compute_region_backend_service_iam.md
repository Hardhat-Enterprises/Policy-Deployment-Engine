## 🛡️ Policy Deployment Engine: `compute_region_backend_service_iam`

This section provides a concise policy evaluation for the `compute_region_backend_service_iam` resource in GCP.

Reference: [Terraform Registry – compute_region_backend_service_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/compute_region_backend_service_iam)
...
|----------|------------|-----------|----------------|-----------|
| `region` | If it is not provided, the provider region is used. Used to find the parent resource to bind the IAM policy to. If not specified, the value will be parsed from the identifier of the parent resource. If no region is provided in the parent identifier and no region is specified, it is taken from the provider configuration. | none | None | None |
| `name` |  | none | None | None |
...
| `role` | `google_compute_region_backend_service_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
