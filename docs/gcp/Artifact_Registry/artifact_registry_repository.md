...
| `kms_key_name` | The Cloud KMS resource name of the customer managed encryption key that’s used to encrypt the contents of the Repository. Has the form: `projects/my-project/locations/my-region/keyRings/my-kr/cryptoKeys/my-key`. This value may not be changed after the Repository has been created. | false | false | Two policies apply: cmek_required requires kms_key_name to be set, and approved_cmek_keys restricts it to approved CMEK path patterns and approved project/location/key identifiers. | ['kms_key_name = "projects/project-1/locations/us-central1/keyRings/artifact-ring/cryptoKeys/artifact-key"', 'kms_key_name = "projects/project-2/locations/australia-southeast1/keyRings/platform-ring/cryptoKeys/repo-key"'] | ['kms_key_name = null', 'kms_key_name = ""', 'kms_key_name = "kms-key"', 'kms_key_name = "projects/other-project/locations/europe-west1/keyRings/test/cryptoKeys/key1"'] |
...
| `mode` | The mode configures the repository to serve artifacts from different sources. Default value is `STANDARD_REPOSITORY`. Possible values are: `STANDARD_REPOSITORY`, `VIRTUAL_REPOSITORY`, `REMOTE_REPOSITORY`. | false | false | The approved_modes policy whitelists STANDARD_REPOSITORY, REMOTE_REPOSITORY, and VIRTUAL_REPOSITORY only. | ['mode = "STANDARD_REPOSITORY"', 'mode = "REMOTE_REPOSITORY"', 'mode = "VIRTUAL_REPOSITORY"'] | ['mode = "UNKNOWN_MODE"', 'mode = "PROXY_REPOSITORY"'] |
...
| `cleanup_policy_dry_run` | If true, the cleanup pipeline is prevented from deleting versions in this repository. | false | false | The cleanup_policy_guardrails policy requires cleanup_policy_dry_run to be true. | ['cleanup_policy_dry_run = true'] | ['cleanup_policy_dry_run = false'] |
...
| `location` | The name of the repository's location. In addition to specific regions, special values for multi-region locations are `asia`, `europe`, and `us`. See [here](https://cloud.google.com/artifact-registry/docs/repositories/repo-locations), or use the [google_artifact_registry_locations](https://registry.terraform.io/providers/hashicorp/google/latest/docs/data-sources/artifact_registry_locations) data source for possible values. | false | false | The approved_locations policy whitelists only australia-southeast1-a/b/c and australia-southeast2-a/b/c. | ['location = "australia-southeast1-a"', 'location = "australia-southeast2-b"'] | ['location = "us-central1"', 'location = "europe-west1"', 'location = "asia"'] |
...
| Argument | Description | Required | Security Impact | Rationale | Compliant | Non-Compliant |
|----------|-------------|----------|-----------------|-----------|-----------|---------------|
| `immutable_tags` | The repository which enabled this flag prevents all tags from being modified, moved or deleted. This does not prevent tags from being created. | false | false | The docker_immutable_tags policy requires immutable_tags = true when format is DOCKER. | ['docker_config { immutable_tags = true }'] | ['docker_config { immutable_tags = false }'] |

### maven_config Block

| Argument | Description | Required | Security Impact | Rationale | Compliant | Non-Compliant |
|----------|-------------|----------|-----------------|-----------|-----------|---------------|
| `allow_snapshot_overwrites` | The repository with this flag will allow publishing the same snapshot versions. | false | false | The maven_snapshot_overwrite_disallowed policy blocks allow_snapshot_overwrites = true when format is MAVEN. | ['maven_config { allow_snapshot_overwrites = false }'] | ['maven_config { allow_snapshot_overwrites = true }'] |
...
| Argument | Description | Required | Security Impact | Rationale | Compliant | Non-Compliant |
|----------|-------------|----------|-----------------|-----------|-----------|---------------|
| `upstream_policies` | Policies that configure the upstream artifacts distributed by the Virtual Repository. Upstream policies cannot be set on a standard repository. Structure is [documented below](#nested_virtual_repository_config_upstream_policies). | false | false | The virtual_upstream_priority_range policy requires upstream_policies.priority to stay within 1 to 100 when mode is VIRTUAL_REPOSITORY. | ['upstream_policies { priority = 1 }', 'upstream_policies { priority = 50 }', 'upstream_policies { priority = 100 }'] | ['upstream_policies { priority = 0 }', 'upstream_policies { priority = 101 }'] |
...
| `action` | Policy action. Possible values are: `DELETE`, `KEEP`. | false | false | The cleanup_policy_guardrails policy forbids DELETE rules targeting TAGGED artifacts and expects KEEP policies to retain at least 3 recent versions. | ['action = "DELETE" with condition.tag_state = "UNTAGGED"', 'action = "KEEP" with most_recent_versions.keep_count = 3'] | ['action = "DELETE" with condition.tag_state = "TAGGED"', 'action = "KEEP" with most_recent_versions.keep_count = 1'] |
| `condition` | Policy condition for matching versions. Structure is [documented below](#nested_cleanup_policies_cleanup_policies_condition). | false | false | Your guardrail policy specifically checks condition.tag_state for DELETE policies and blocks TAGGED artifacts from matching deletion. | ['condition { tag_state = "UNTAGGED" }'] | ['condition { tag_state = "TAGGED" }'] |
| `most_recent_versions` | Policy condition for retaining a minimum number of versions. May only be specified with a Keep action. Structure is [documented below](#nested_cleanup_policies_cleanup_policies_most_recent_versions). | false | false | Your guardrail policy requires KEEP policies to retain at least 3 recent versions via most_recent_versions.keep_count. | ['most_recent_versions { keep_count = 3 }', 'most_recent_versions { keep_count = 10 }'] | ['most_recent_versions { keep_count = 1 }', 'most_recent_versions { keep_count = 2 }'] |
...
| `common_repository` | Specific settings for an Artifact Registory remote repository. Structure is [documented below](#nested_remote_repository_config_common_repository). | false | false | The approved_remote_upstreams policy allows only approved upstream Artifact Registry URIs for REMOTE_REPOSITORY mode. | ['common_repository { uri = "https://us-central1-docker.pkg.dev/trusted-project/base-images" }', 'common_repository { uri = "https://australia-southeast1-docker.pkg.dev/shared-artifacts/approved-upstream" }'] | ['common_repository { uri = "https://docker.io/library" }', 'common_repository { uri = "https://europe-west1-docker.pkg.dev/unknown-project/test" }'] |
...
| `disable_upstream_validation` | If true, the remote repository upstream and upstream credentials will not be validated. | false | false | The remote_upstream_validation_required policy blocks disable_upstream_validation = true for REMOTE_REPOSITORY mode. | ['disable_upstream_validation = false'] | ['disable_upstream_validation = true'] |

### vulnerability_scanning_config Block

| Argument | Description | Required | Security Impact | Rationale | Compliant | Non-Compliant |
|----------|-------------|----------|-----------------|-----------|-----------|---------------|
| `enablement_config` | This configures whether vulnerability scanning is automatically performed for artifacts pushed to this repository. Possible values are: `INHERITED`, `DISABLED`. | false | false | The vulnerability_scanning_enabled policy requires enablement_config to resolve to INHERITED rather than DISABLED. | ['vulnerability_scanning_config { enablement_config = "INHERITED" }'] | ['vulnerability_scanning_config { enablement_config = "DISABLED" }'] |
...
| `priority` | Entries with a greater priority value take precedence in the pull order. | false | false | The virtual_upstream_priority_range policy requires priority to stay within 1 to 100. | ['priority = 1', 'priority = 50', 'priority = 100'] | ['priority = 0', 'priority = 101'] |

### condition Block

| Argument | Description | Required | Security Impact | Rationale | Compliant | Non-Compliant |
|----------|-------------|----------|-----------------|-----------|-----------|---------------|
| `tag_state` | Match versions by tag status. Default value is `ANY`. Possible values are: `TAGGED`, `UNTAGGED`, `ANY`. | false | false | The cleanup_policy_guardrails policy blocks DELETE cleanup rules from using tag_state = TAGGED. | ['tag_state = "UNTAGGED"', 'tag_state = "ANY" for non-delete review scenarios'] | ['tag_state = "TAGGED" under a DELETE action'] |
...
|----------|-------------|----------|-----------------|-----------|-----------|---------------|
| `package_name_prefixes` | Match versions by package prefix. Applied on any prefix match. | false | false | None | None | None |
| `keep_count` | Minimum number of versions to keep. | false | false | The cleanup_policy_guardrails policy enforces a keep_count range of 3 to 100 for the checked KEEP policy. | ['keep_count = 3', 'keep_count = 5', 'keep_count = 100'] | ['keep_count = 1', 'keep_count = 2', 'keep_count = 101'] |
