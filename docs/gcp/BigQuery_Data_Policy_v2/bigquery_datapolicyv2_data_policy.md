...
| `location` | Resource location for the data policy. | true | true | Location can be restricted to approved regions. | Set `location` to an approved AU region such as `australia-southeast1`. | Set `location` to an unapproved region such as `europe-west1`. |
| `data_masking_policy` | The policy block used to specify a masking rule. | false | true | Masking rules affect how data is shown. | Set `data_masking_policy` with an approved predefined expression or approved routine. | Use a weak masking expression or an unapproved routine. |
