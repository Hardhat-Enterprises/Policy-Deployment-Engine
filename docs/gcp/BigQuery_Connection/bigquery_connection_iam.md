## 🛡️ Policy Deployment Engine: `bigquery_connection_iam`

This section provides a concise policy evaluation for the `bigquery_connection_iam` resource in GCP.

Reference: [Terraform Registry – bigquery_connection_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/bigquery_connection_iam)
...
| `role` | `google_bigquery_connection_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
