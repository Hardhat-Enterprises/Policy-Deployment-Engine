...
| `can_ip_forward` | Allows the TPU node to send and receive packets with non-matching destination or source IPs. This is required if you plan to use the TPU workers to forward routes. | false | false | IP forwarding can increase the attack surface by allowing packets with non-matching source or destination IPs. This attribute is security relevant but the policy has been implemented in google_tpu_v2_queued_resource. | None | None |
