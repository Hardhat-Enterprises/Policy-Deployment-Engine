...
| `location` | The location for the resource | true | true | The location determines where data is stored and processed. Selecting an incorrect region may violate data residency requirements. | us-central1 | None |
...
| `data_logs_viewer` | Allow the partner to view inspectability logs and monitoring violations. | false | true | Access to inspectability logs enables partners to audit data access and detect anomalies. | None | None |
| `service_access_approver` | Optional. Allow partner to view access approval logs. | false | true | Access approval logs provide an audit trail of who accessed customer data, supporting accountability and compliance auditing. | None | None |
