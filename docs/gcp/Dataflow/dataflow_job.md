...
| `template_gcs_path` | The GCS path to the Dataflow job template. | true | false | None | None | None |
| `temp_gcs_location` | A writeable location on GCS for the Dataflow job to dump its temporary data. | true | false | None | None | None |
...
| `max_workers` | The number of workers permitted to work on the job. More workers may improve processing speed at additional cost. | false | false | None | None | None |
...
| `zone` | The zone in which the created job should run. If it is not provided, the provider zone is used. | false | false | None | None | None |
