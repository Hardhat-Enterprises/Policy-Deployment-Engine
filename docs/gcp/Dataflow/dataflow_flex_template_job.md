...
|----------|-------------|----------|-----------------|-----------|-----------|---------------|
| `name` | A unique name for the resource, required by Dataflow. | true | false | None | None | None |
| `container_spec_gcs_path` | The GCS path to the Dataflow Flex Template spec file. | true | false | None | None | None |
| `additional_experiments` | List of experiments that should be used by the job. | false | false | None | None | None |
| `additional_pipeline_options` | Additional pipeline options to pass to the Flex Template job. | false | false | None | None | None |
| `autoscaling_algorithm` | The algorithm to allow Dataflow to scale between minWorkers and maxWorkers. | false | false | None | None | None |
| `parameters` | Key/value pairs to be passed to the Dataflow job (as used in the template). Keys are case-sensitive based on the language on which the pipeline is coded, mostly Java. Note: do not configure Dataflow options here in parameters. | false | false | None | None | None |
...
| `launcher_machine_type` | The machine type to use for launching the job. The default is n1-standard-1. | false | false | None | None | None |
| `machine_type` | The machine type to use for the job. | false | false | None | None | None |
| `max_workers` | The maximum number of Google Compute Engine instances to be made available to your pipeline during execution, from 1 to 1000. | false | false | None | None | None |
...
| `num_workers` | The initial number of Google Compute Engine instances for the job. | false | false | None | None | None |
| `on_delete` | One of 'drain' or 'cancel'. Specifies behavior of deletion during terraform destroy. | false | false | None | None | None |
| `project` | The project in which the resource belongs. If it is not provided, the provider project is used. | false | false | None | None | None |
| `region` | The region in which the created job should run. | false | false | None | None | None |
| `sdk_container_image` | Docker registry location of container image to use for the 'worker harness'. Default is the container for the version of the SDK. | false | false | None | None | None |
...
| `skip_wait_on_job_termination` | If true, treat DRAINING and CANCELLING as terminal states and do not wait for further changes before removing from terraform state and moving on. | false | false | None | None | None |
| `staging_location` | The GCS path for staging files. A random subdirectory will be created under the bucket for job staging. | false | false | None | None | None |
| `subnetwork` | The subnetwork to which VMs will be assigned. Should be of the form 'regions/REGION/subnetworks/SUBNETWORK'. | false | false | None | None | None |
| `temp_location` | The GCS path for temporary files. A random subdirectory will be created under the bucket for temp data. | false | false | None | None | None |
| `transform_name_mapping` | Only applicable when updating a pipeline. Map of transform name prefixes of the job to be replaced with the corresponding name prefixes of the new job. | false | false | None | None | None |
