## 🛡️ Policy Deployment Engine: `secret_manager_regional_secret_iam`

This section provides a concise policy evaluation for the `secret_manager_regional_secret_iam` resource in GCP.

Reference: [Terraform Registry – secret_manager_regional_secret_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/secret_manager_regional_secret_iam)
...
| `role` | `google_secret_manager_regional_secret_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
