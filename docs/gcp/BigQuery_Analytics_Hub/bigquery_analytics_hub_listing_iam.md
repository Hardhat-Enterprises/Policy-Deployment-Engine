...
| `data_exchange_id` |  | none | None | None |
| `listing_id` |  | none | None | None |
...
| `role` | `google_bigquery_analytics_hub_listing_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
