...
### `source_restriction`
- Description: (Optional) Whether to enforce traffic restrictions based on `sources` field. If the `sources` field is non-empty, then this field must be set to `SOURCE_RESTRICTION_ENABLED`. Possible values are: `SOURCE_RESTRICTION_ENABLED`, `SOURCE_RESTRICTION_DISABLED`. <a name="nested_egress_from_sources"></a>The `sources` block supports:
- Required: 
- Policy Condition?: 
- Decision / Rationale: 
