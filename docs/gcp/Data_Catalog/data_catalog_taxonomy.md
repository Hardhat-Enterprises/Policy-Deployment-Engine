...
| `description` | Description of this taxonomy. It must: contain only unicode characters, tabs, newlines, carriage returns and page breaks; and be at most 2000 bytes long when encoded in UTF-8. If not set, defaults to an empty description. | false | true | A clear description is needed so the purpose of the taxonomy can be audited and understood. | Taxonomy for sensitive data classification | None |
...
| `project` | If it is not provided, the provider project is used. | false | true | Taxonomies should be created only in approved projects to keep classification resources inside the correct boundary. | gcp-project-12345 | unapproved-project |
