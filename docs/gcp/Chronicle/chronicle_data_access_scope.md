...
| `instance` | The unique identifier for the Chronicle instance, which is the same as the customer ID. | true | true | The instance ID uniquely identifies the Chronicle customer. Incorrect configuration may lead to unauthorized data access or failed API interactions. | abcd1234 |  |
| `data_access_scope_id` | Required. The user provided scope id which will become the last part of the name of the scope resource. Needs to be compliant with https://google.aip.dev/122 | true | true | Improper scope IDs may break resource naming conventions and lead to failed deployments or policy mismatches. Following AIP-122 ensures interoperability and consistency. | scope-label-01 | Scope_Label@123 |
