...
| `resource_type` | The type of resource referred to by this reference. Valid values are 'KeyStore' or 'TrustStore'. | true | true | Controls which type of certificate store is referenced. KeyStore holds private keys and certificates used for TLS/SSL. TrustStore holds trusted certificates for verifying connections. Using an invalid type can misconfigure TLS/SSL, leading to security vulnerabilities such as man-in-the-middle attacks. | KeyStore | InvalidType |
