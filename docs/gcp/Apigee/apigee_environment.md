...
| `deployment_type` | Deployment type supported by the environment. Possible values are: DEPLOYMENT_TYPE_UNSPECIFIED, PROXY, ARCHIVE. | false | false | It sets the deployment type and does not have a direct security impact. | None | None |
| `api_proxy_type` | API Proxy type supported by the environment. Possible values are: API_PROXY_TYPE_UNSPECIFIED, PROGRAMMABLE, CONFIGURABLE. | false | false | It sets the proxy type and does not have any direct security impact. | None | None |
| `type` | Environment type. Possible values are: ENVIRONMENT_TYPE_UNSPECIFIED, BASE, INTERMEDIATE, COMPREHENSIVE. | false | false | It sets the environment tier and does not have any direct security impact. | None | None |
