...
| `deployment_type` | Deployment type supported by the environment. Possible values are: DEPLOYMENT_TYPE_UNSPECIFIED, PROXY, ARCHIVE. | false | true | Setting deployment_type to DEPLOYMENT_TYPE_UNSPECIFIED may lead to unpredictable deployment behaviour and misconfigured environments. Always specify PROXY or ARCHIVE explicitly. | PROXY | DEPLOYMENT_TYPE_UNSPECIFIED |
| `api_proxy_type` | API Proxy type supported by the environment. Possible values are: API_PROXY_TYPE_UNSPECIFIED, PROGRAMMABLE, CONFIGURABLE. | false | true | Setting api_proxy_type to API_PROXY_TYPE_UNSPECIFIED does not enforce a defined proxy execution model which can lead to misconfigured API proxies. Always specify PROGRAMMABLE or CONFIGURABLE explicitly. | PROGRAMMABLE | API_PROXY_TYPE_UNSPECIFIED |
| `type` | Environment type. Possible values are: ENVIRONMENT_TYPE_UNSPECIFIED, BASE, INTERMEDIATE, COMPREHENSIVE. | false | true | Setting type to ENVIRONMENT_TYPE_UNSPECIFIED does not enforce a defined capability and capacity tier which may lead to misconfigured environments. Always specify BASE, INTERMEDIATE or COMPREHENSIVE explicitly. | INTERMEDIATE | ENVIRONMENT_TYPE_UNSPECIFIED |
