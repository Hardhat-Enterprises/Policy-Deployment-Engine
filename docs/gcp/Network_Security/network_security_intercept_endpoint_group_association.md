...
|----------|------------|-----------|----------------|-----------|
| `intercept_endpoint_group` | The endpoint group that this association is connected to, for example: `projects/123456789/locations/global/interceptEndpointGroups/my-eg`. See https://google.aip.dev/124. | true | None | None |
...
| `intercept_endpoint_group_association_id` | The ID to use for the new association, which will become the final component of the endpoint group's resource name. If not provided, the server will generate a unique ID. | false | None | None |
