...
### `name`
- Description: (Required) Name of the BackendAuthenticationConfig resource.
- Required: 
- Policy Condition?: 
- Decision / Rationale: 

### `labels`
- Description: (Optional) Set of label tags associated with the BackendAuthenticationConfig resource. **Note**: This field is non-authoritative, and will only manage the labels present in your configuration. Please refer to the field `effective_labels` for all of the labels present on the resource.
- Required: 
- Policy Condition?: 
- Decision / Rationale: 
...
### `client_certificate`
- Description: (Optional) Reference to a Certificate resource from the certificatemanager.googleapis.com namespace. Used by a BackendService to negotiate mTLS when the backend connection uses TLS and the backend requests a client certificate. Must have a CLIENT_AUTH scope.
- Required: 
- Policy Condition?: 
- Decision / Rationale: 
...
### `location`
- Description: (Optional) The location of the backend authentication config. The default value is `global`.
- Required: 
- Policy Condition?: 
- Decision / Rationale: 
