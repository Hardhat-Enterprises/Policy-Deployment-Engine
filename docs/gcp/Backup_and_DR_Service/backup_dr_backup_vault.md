...
| `backup_vault_id` | Required. ID of the requesting object. | true | false |  Vault ID has no impact on the security of the resource or data contained. | None | None |
...
| `force_delete` | (Deprecated) If set, the following restrictions against deletion of the backup vault instance can be overridden: * deletion of a backup vault instance containing no backups, but still containing empty datasources. * deletion of a backup vault instance that is being referenced by an active backup plan. ~> **Warning:** `force_delete` is deprecated and will be removed in a future major release. Use `ignore_inactive_datasources` instead. | false | false | fDeletion override option; not enforced by policy. | None | None |
...
| `project` | If it is not provided, the provider project is used. | false | false | Inheritance ensures retention policies defined at the backup vault level are consistently enforced. | None | None |
