...
|----------|------------|-----------|----------------|-----------|
| `folder` | The folder where Cloud Security Command Center Big Query Export Config lives in. | true | None | None |
| `big_query_export_id` | This must be unique within the organization.  It must consist of only lowercase letters, numbers, and hyphens, must start with a letter, must end with either a letter or a number, and must be 63 characters or less. | true | None | None |
...
| `location` | The BigQuery export configuration is stored in this location. If not provided, Use global as default. | false | None | None |
