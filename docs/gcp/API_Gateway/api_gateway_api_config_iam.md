...
|----------|------------|-----------|----------------|-----------|
| `api` | Used to find the parent resource to bind the IAM policy to | none | None | None |
| `api_config` |  | none | None | None |
...
| `role` | `google_api_gateway_api_config_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
