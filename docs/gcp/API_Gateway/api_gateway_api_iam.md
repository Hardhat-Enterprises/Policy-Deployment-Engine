## 🛡️ Policy Deployment Engine: `api_gateway_api_iam`

This section provides a concise policy evaluation for the `api_gateway_api_iam` resource in GCP.

Reference: [Terraform Registry – api_gateway_api_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/api_gateway_api_iam)
...
|----------|-------------|----------|-----------------|-----------|-----------|---------------|
| `api` |  | false | false | None | None | None |
...
| `role` | `google_api_gateway_api_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | false | false | None | None | None |
