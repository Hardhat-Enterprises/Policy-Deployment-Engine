## 🛡️ Policy Deployment Engine: `kms_ekm_connection_iam`

This section provides a concise policy evaluation for the `kms_ekm_connection_iam` resource in GCP.

Reference: [Terraform Registry – kms_ekm_connection_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/kms_ekm_connection_iam)
...
| `role` | `google_kms_ekm_connection_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
