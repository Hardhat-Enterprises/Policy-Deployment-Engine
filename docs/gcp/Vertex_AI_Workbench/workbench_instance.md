...
| `labels` | Optional. Labels to apply to this instance. These can be later modified by the UpdateInstance method. **Note**: This field is non-authoritative, and will only manage the labels present in your configuration. Please refer to the field `effective_labels` for all of the labels present on the resource. | false | true | Labels are required for cost allocation, ownership tracking, and security policy enforcement. Without labels, resources cannot be properly governed or audited. | labels = { environment = "dev", team = "data-science" } | labels omitted |
