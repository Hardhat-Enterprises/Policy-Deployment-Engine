## 🛡️ Policy Deployment Engine: `gke_hub_scope_iam`

This section provides a concise policy evaluation for the `gke_hub_scope_iam` resource in GCP.

Reference: [Terraform Registry – gke_hub_scope_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/gke_hub_scope_iam)
...
| `role` | `google_gke_hub_scope_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
