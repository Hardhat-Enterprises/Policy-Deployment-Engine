...
| `instance` | For `google_bigtable_instance_iam_member` or `google_bigtable_instance_iam_binding`: | none | None | None |
...
| `role` | `google_bigtable_instance_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. Read more about roles [here](https://cloud.google.com/bigtable/docs/access-control#roles). | none | None | None |
...
| `description` | For `google_bigtable_instance_iam_policy` only: | none | None | None |
