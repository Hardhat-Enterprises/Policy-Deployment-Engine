...
| `deletion_protection` | `PROTECTED` prohibits deleting the table, column families, or the instance containing the table via the API. Defaults to `UNPROTECTED` if unset. | false | false | API-side safeguard against destructive deletes; it does not publish rows publicly—availability/governance oriented rather than client authentication. | None | None |
...
| `automated_backup_policy` | Automated backup retention and frequency. Omit or set periods/frequency to zero to disable backups. | false | true | Scheduled backups support recovery and incident response for table data. | { retention_period = "72h", frequency = "24h" } | missing block, or retention_period = "0", or frequency = "0" |
