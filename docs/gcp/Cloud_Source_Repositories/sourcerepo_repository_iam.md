## 🛡️ Policy Deployment Engine: `sourcerepo_repository_iam`

This section provides a concise policy evaluation for the `sourcerepo_repository_iam` resource in GCP.

Reference: [Terraform Registry – sourcerepo_repository_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/sourcerepo_repository_iam)
...
| `role` | `google_sourcerepo_repository_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
