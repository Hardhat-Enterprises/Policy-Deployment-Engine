## 🛡️ Policy Deployment Engine: `dns_managed_zone_iam`

This section provides a concise policy evaluation for the `dns_managed_zone_iam` resource in GCP.

Reference: [Terraform Registry – dns_managed_zone_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/dns_managed_zone_iam)
...
| `role` | `google_dns_managed_zone_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
