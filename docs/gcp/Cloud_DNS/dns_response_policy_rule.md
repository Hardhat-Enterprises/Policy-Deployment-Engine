...
| `dns_name` | The DNS name (wildcard or exact) to apply this rule to. Must be unique within the Response Policy Rule. | true | true | Wildcard DNS names are less secure because they apply rules broadly across multiple hostnames, increasing the risk of unintended matches, misconfigurations, and overly permissive DNS policy enforcement. A pattern blacklist policy was investigated to detect wildcard dns_name values however a bug in the shared.rego final_formatter function causes regex.replace to fail when the blacklisted value is `*` as it is an invalid standalone RE2 regex pattern. This has been raised with the project lead for resolution. | api.example.com. | *.example.com. |
