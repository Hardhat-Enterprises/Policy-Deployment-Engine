...
| `dnssec_config` | DNSSEC configuration. Structure is documented below. | false | true | DNSSEC must be enabled to cryptographically sign DNS responses, preventing DNS spoofing and cache poisoning attacks. A managed zone without DNSSEC enabled cannot authenticate its DNS responses, leaving it vulnerable to man-in-the-middle attacks. | state = "on" | state = "off" |
