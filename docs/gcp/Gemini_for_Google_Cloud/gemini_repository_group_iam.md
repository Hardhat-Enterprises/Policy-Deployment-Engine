...
| `code_repository_index` |  | none | None | None |
| `repository_group_id` |  | none | None | None |
...
| `role` | `google_gemini_repository_group_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
