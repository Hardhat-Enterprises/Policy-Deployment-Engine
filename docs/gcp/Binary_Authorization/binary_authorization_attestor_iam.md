## 🛡️ Policy Deployment Engine: `binary_authorization_attestor_iam`

This section provides a concise policy evaluation for the `binary_authorization_attestor_iam` resource in GCP.

Reference: [Terraform Registry – binary_authorization_attestor_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/binary_authorization_attestor_iam)
...
| `role` | `google_binary_authorization_attestor_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
