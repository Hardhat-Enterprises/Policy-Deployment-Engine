...
| `cron_schedule` | Optional. Optional schedule (in cron format) for automatic creation of compilation results. | false | true | Automated scheduling ensures consistent data processing and reduces manual intervention risks while maintaining data freshness. | "0 2 * * *" (daily at 2 AM) | null or empty (no automated scheduling) |
