...
| `service_account` | Optional. The service account to run workflow invocations under. | false | true | Explicit service account configuration ensures proper identity and access management for workflow executions, following the principle of least privilege. | dataform-workflow-sa@project.iam.gserviceaccount.com | null or empty (uses default identity) |
