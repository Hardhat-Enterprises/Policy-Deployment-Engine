...
|----------|-------------|----------|-----------------|-----------|-----------|---------------|
| `folder` | The folder that will own the exclusion filter. Format: folders/FOLDER_ID | true | false | This identifies which folder owns the exclusion filter. It has no direct security impact on the exclusion's behavior. | folders/1234567890 | None |
| `name` | The name of the logging exclusion. Must be unique within the folder. | true | false | Name is a unique identifier for the exclusion filter. It has no security implications beyond being a reference name. | audit-log-exclusion | None |
