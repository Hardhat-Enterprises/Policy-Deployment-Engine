...
| `feature_online_store` |  | none | None | None |
| `feature_view` |  | none | None | None |
...
| `role` | `google_vertex_ai_feature_online_store_featureview_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
