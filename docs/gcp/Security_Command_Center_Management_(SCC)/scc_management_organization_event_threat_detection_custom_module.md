## 🛡️ Policy Deployment Engine: `scc_management_organization_event_threat_detection_custom_module`

This section provides a concise policy evaluation for the `scc_management_organization_event_threat_detection_custom_module` resource in GCP.

Reference: [Terraform Registry – scc_management_organization_event_threat_detection_custom_module](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/scc_management_organization_event_threat_detection_custom_module)

---

## Argument Reference
| Argument | Description | Mandatory | Security Impact | Rationale |
|----------|------------|-----------|----------------|-----------|
| `organization` | Numerical ID of the parent organization. | true | None | None |
| `config` | Config for the module. For the resident module, its config value is defined at this level. For the inherited module, its config value is inherited from the ancestor module. | false | None | None |
| `enablement_state` | The state of enablement for the module at the given level of the hierarchy. Possible values are: `ENABLED`, `DISABLED`. | false | None | None |
| `type` | Immutable. Type for the module. e.g. CONFIGURABLE_BAD_IP. | false | None | None |
| `display_name` | The human readable name to be displayed for the module. | false | None | None |
| `location` | Location ID of the parent organization. Only global is supported at the moment. | false | None | None |
