...
| `certificate_template` |  | false | false | Reference to the certificate template resource being bound; does not configure a security-enforceable property. | None | None |
...
| `role` | `google_privateca_certificate_template_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | false | false | IAM role binding; the appropriate role is deployment-specific and no specific role value is mandated by policy. | None | None |
