...
| `certificate_template` |  | false | false | None | None | None |
...
| `role` | `google_privateca_certificate_template_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | false | false | None | None | None |
