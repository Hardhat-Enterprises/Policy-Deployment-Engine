## 🛡️ Policy Deployment Engine: `privateca_certificate_template_iam`

This section provides a concise policy evaluation for the `privateca_certificate_template_iam` resource in GCP.

Reference: [Terraform Registry – privateca_certificate_template_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/privateca_certificate_template_iam)
...
|----------|------------|-----------|----------------|-----------|
| `location` | the value will be parsed from the identifier of the parent resource. If no location is provided in the parent identifier and no location is specified, it is taken from the provider configuration. | none | None | None |
| `certificate_template` |  | none | None | None |
...
| `role` | `google_privateca_certificate_template_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | none | None | None |
