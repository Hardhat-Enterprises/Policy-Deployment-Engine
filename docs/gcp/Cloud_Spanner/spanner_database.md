...
|----------|------------|-----------|----------------|-----------|
| `name` | A unique identifier for the database, which cannot be changed after the instance is created. Values are of the form `[a-z][-_a-z0-9]*[a-z0-9]`. | true | None | None |
...
| `default_time_zone` | from the tz database. Default value is "America/Los_angeles". | none | None | None |

### encryption_config Block
| Argument | Description | Mandatory | Security Impact | Rationale |
|----------|------------|-----------|----------------|-----------|
| `kms_key_name` | Fully qualified name of the KMS key to use to encrypt this database. This key must exist in the same location as the Spanner Database. | false | None | None |
