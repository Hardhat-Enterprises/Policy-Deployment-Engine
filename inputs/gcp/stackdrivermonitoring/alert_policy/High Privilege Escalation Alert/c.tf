...
# High Privilege Escalation Alert Policy - COMPLIANT
resource "google_monitoring_alert_policy" "privilege_escalation_c" {
  display_name = "Privilege Escalation Detected - Compliant"
  project      = google_project.project-c.project_id
  combiner     = "OR"
  ...
  notification_channels = [
    ...
    google_monitoring_notification_channel.critical_alerts_c.name
  ]
  ...
  documentation {
    content = "High-priority security alert for privilege escalation attempts. Requires immediate security team response."
