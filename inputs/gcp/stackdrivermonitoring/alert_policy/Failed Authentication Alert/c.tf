...
# Failed Authentication Alert Policy - COMPLIANT
resource "google_monitoring_alert_policy" "failed_auth_alert_c" {
  display_name = "High Failed Authentication Rate - Compliant"
  project      = google_project.project-c.project_id
  combiner     = "OR"
  ...
  notification_channels = [google_monitoring_notification_channel.security_team_c.name]
  ...
  documentation {
    content = "Alert triggered when failed authentication attempts exceed threshold. Security team should investigate immediately."
