...
resource "google_secret_manager_secret_iam_binding" "c" {
  ...
  condition {
    ...
    description = "Enforce weekday and expiration restriction"
    expression  = "request.time < timestamp(\"2026-01-01T00:00:00Z\") && request.time.getDayOfWeek() >= 1 && request.time.getDayOfWeek() <= 5"
  }
}
