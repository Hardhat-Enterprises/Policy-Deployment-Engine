...
#
# deletion_protection is set to false so Terraform is allowed to destroy this
# cluster without any safeguard. All non-tested fields are identical to
# compliant.tf so the only variable under test is deletion_protection.
