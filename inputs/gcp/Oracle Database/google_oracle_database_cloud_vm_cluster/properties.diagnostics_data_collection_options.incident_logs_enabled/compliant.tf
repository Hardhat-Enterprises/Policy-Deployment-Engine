...
#
# incident_logs_enabled is set to false so incident logs and traces are not
# collected. All non-tested fields are identical to nonCompliant.tf so the
# only variable under test is incident_logs_enabled.
