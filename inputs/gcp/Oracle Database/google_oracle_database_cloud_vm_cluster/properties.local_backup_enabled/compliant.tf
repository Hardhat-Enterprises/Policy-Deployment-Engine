...
#
# local_backup_enabled is set to true so the cluster retains a local backup
# copy it can be restored from after a destructive event. All non-tested
# fields are identical to nonCompliant.tf so the only variable under test is
