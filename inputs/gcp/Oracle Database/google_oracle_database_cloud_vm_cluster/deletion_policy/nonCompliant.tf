...
#
# deletion_policy is set to "DELETE" so Terraform is allowed to destroy this
# cluster without protection. All non-tested fields are identical to
# compliant.tf so the only variable under test is deletion_policy.
