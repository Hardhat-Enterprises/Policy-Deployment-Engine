...
#
# deletion_policy is set to "PREVENT" so Terraform will refuse to destroy
# this cluster. All non-tested fields are identical to nonCompliant.tf so the
# only variable under test is deletion_policy.
