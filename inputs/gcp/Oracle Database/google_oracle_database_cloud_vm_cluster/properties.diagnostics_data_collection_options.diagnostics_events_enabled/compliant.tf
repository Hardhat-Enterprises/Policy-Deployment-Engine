...
#
# diagnostics_events_enabled is set to false so diagnostic event metadata is
# not collected. All non-tested fields are identical to nonCompliant.tf so
# the only variable under test is diagnostics_events_enabled.
