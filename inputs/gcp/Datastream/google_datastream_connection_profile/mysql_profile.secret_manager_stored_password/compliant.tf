...
# The MySQL password references the latest Secret Manager version.
...
resource "google_datastream_connection_profile" "compliant_example_1" {
  ...
  mysql_profile {
    ...
    secret_manager_stored_password = "projects/example-project/secrets/mysql-password/versions/latest"
  }
}
