...
# The SQL Server password references the latest Secret Manager version.
...
resource "google_datastream_connection_profile" "compliant_example_1" {
  ...
  sql_server_profile {
    ...
    secret_manager_stored_password = "projects/example-project/secrets/sql-server-password/versions/latest"
  }
}
