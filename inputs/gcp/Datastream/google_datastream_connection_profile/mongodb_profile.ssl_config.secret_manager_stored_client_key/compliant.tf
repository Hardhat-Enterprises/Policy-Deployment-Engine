...
# The MongoDB TLS client key references the latest Secret Manager version.
...
resource "google_datastream_connection_profile" "compliant_example_1" {
  ...
  mongodb_profile {
    ...
    ssl_config {
      secret_manager_stored_client_key = "projects/example-project/secrets/mongodb-client-key/versions/latest"
    }
  }
}
