...
violating_resources contains name if {
    ...
    object.get(conn, "private_key", null) == null
}

message := [
    "Situation 1: oracle.forward_ssh_connectivity.private_key is not configured.",
    sprintf("Non-Compliant Resources: %s", [concat(", ", violating_resources)]),
    "Potential Remedies: Set oracle.forward_ssh_connectivity.private_key when Oracle forward SSH connectivity is used.",
] if {
