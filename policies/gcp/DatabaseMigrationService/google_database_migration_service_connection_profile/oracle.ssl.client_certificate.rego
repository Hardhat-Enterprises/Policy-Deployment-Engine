package terraform.gcp.security.database_migration_service.google_database_migration_service_connection_profile.oracle_ssl_client_certificate
...
violating_resources contains name if {
    ...
    ssl := resource.values.oracle[0].ssl[0]
    object.get(ssl, "client_certificate", null) == null
}

message := [
    "Situation 1: oracle.ssl.client_certificate is not configured.",
    sprintf("Non-Compliant Resources: %s", [concat(", ", violating_resources)]),
    "Potential Remedies: Set oracle.ssl.client_certificate to support SSL verification or client certificate authentication.",
] if {
