...
violating_resources contains name if {
    ...
    ssl := resource.values.mysql[0].ssl[0]
    object.get(ssl, "client_key", null) == null
}

message := [
    "Situation 1: MySQL SSL client key is not configured.",
    sprintf("Non-Compliant Resources: %s", [concat(", ", violating_resources)]),
    "Potential Remedies: Set mysql.ssl.client_key when client certificate authentication is used.",
] if {
