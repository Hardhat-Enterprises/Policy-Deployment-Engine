    ...
    [
        {
            ...
            "remedies":  package terraform.gcp.security.compute_engine.google_compute_disk.zone
import data.terraform.helpers
import data.terraform.gcp.security.compute_engine.google_compute_disk.vars
conditions := [
    [
...
details := result.details.Groups[1].Value -replace '\.','_' 
        ...
        {
            "condition": "zone must be in an approved Australian region.",
            "attribute_path":  package terraform.gcp.security.compute_engine.google_compute_disk.zone
import data.terraform.helpers
import data.terraform.gcp.security.compute_engine.google_compute_disk.vars
conditions := [
    [
