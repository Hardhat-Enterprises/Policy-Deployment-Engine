package terraform.gcp.security.organization_policy.vpc_service_controls

import data.terraform.gcp.helpers
import data.terraform.gcp.security.organization_policy.vars

conditions := [
  [
    {
      "situation_description": "VPC Service Controls must be enabled to protect sensitive resources",
      "remedies": ["Set enforce = true for accessPolicies.restrictVpcServiceControls"]
    },
    {
      "condition": "Check if enforce is true",
      "attribute_path": ["spec", 0, "rules", 0, "enforce"],
      "values": [true],
      "policy_type": "whitelist"
    }
  ]
]

message := helpers.get_multi_summary(conditions, vars.variables).message
details := helpers.get_multi_summary(conditions, vars.variables).details
