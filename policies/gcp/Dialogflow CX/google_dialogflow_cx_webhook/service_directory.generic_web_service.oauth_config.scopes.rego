...
conditions := [[
	{
		"situation_description": "The Service Directory webhook requests missing, wildcard-like, administrative, or known overly broad OAuth scopes",
	...
	},
	{
		...
		"attribute_path": ["service_directory", 0, "generic_web_service", 0, "oauth_config", 0, "scopes"],
		"values": [
			null,
			[],
			"",
...
oauth_resources := [normalised |
	...
	oauth := object.get(resource.values, ["service_directory", 0, "generic_web_service", 0, "oauth_config"], [])
	is_array(oauth)
	count(oauth) > 0
	scopes := object.get(oauth[0], "scopes", null)
	normalised := json.patch(resource, [{
		"op": "add",
		"path": ["values", "service_directory", 0, "generic_web_service", 0, "oauth_config", 0, "scopes"],
