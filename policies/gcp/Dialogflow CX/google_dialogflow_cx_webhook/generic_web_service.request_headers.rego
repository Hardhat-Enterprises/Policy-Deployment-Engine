...
violating_resources := {resource |
	...
	headers := object.get(resource.values, ["generic_web_service", 0, "request_headers"], {})
	is_object(headers)
...
message := [
	...
	"Situation 1: The generic webhook contains a secret-bearing inline request header",
...
details := [{
	"situation": "The generic webhook contains a secret-bearing inline request header",
