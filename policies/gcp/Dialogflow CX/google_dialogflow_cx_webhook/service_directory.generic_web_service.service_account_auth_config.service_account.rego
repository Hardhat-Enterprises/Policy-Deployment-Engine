...
normalise_account(resource) := normalised if {
	...
	account := object.get(resource.values, ["service_directory", 0, "generic_web_service", 0, "service_account_auth_config", 0, "service_account"], null)
	is_string(account)
	normalised := json.patch(resource, [{
		"op": "replace",
		"path": ["values", "service_directory", 0, "generic_web_service", 0, "service_account_auth_config", 0, "service_account"],
		"value": lower(account),
	}])
} else := resource
