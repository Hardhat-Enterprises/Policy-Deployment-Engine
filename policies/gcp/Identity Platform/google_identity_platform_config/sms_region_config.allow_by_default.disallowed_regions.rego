...
non_compliant_resource(resource) if {
	...
	regions := object.get(allow_by_default_config, "disallowed_regions", [])
	is_array(regions)
...
message := [
	...
	"Situation 1: A blocked SMS region is not a valid global two-letter region code.",
...
details := [{
	"situation": "A blocked SMS region is not a valid global two-letter region code.",
