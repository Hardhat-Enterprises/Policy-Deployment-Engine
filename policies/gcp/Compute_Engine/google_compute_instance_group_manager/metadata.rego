...
conditions := [
  ...
  [
    # NOTE: narrow, key-name-specific check. It only catches a plaintext
    # secret stored under the literal key "password" - it can't detect the
    # same mistake under any other key name (e.g. "pwd", "api_key"). Generic
    # detection of secret-looking values across arbitrary metadata keys
    # isn't expressible with the current policy_type primitives (they all
    # target a fixed attribute_path, not "any key in this map"), so this is
    # a best-effort spot-check, not full coverage.
    {
