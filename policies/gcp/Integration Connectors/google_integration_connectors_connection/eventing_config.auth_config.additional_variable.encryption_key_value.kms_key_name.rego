...
conditions := [
  [
    {
      "situation_description": "No customer-managed KMS key is nominated for the eventing auth variable, so the value falls back to a Google-managed key and the organisation controls neither its rotation nor its revocation.",
      "remedies": [
