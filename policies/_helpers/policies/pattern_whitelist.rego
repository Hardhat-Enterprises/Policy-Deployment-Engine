...
get_violations(resource_type, attribute_path, values_formatted, friendly_resource_name, value_name) = results if {
    ...
    nc := _get_whitelist(this_nc_resource, attribute_path, values_formatted[0], values_formatted[1])
    this_nc := nc[_]
...
_get_whitelist(resource, attribute_path, target, patterns) = ncc if {
    ...
    ncc := [
        {"value": target_list[i], "allowed": patterns[i]} |
            some i
            not shared.array_contains(patterns[i], target_list[i], "whitelist") # direct mapping of positions of target * with its list of allowed patterns
    ]
}
