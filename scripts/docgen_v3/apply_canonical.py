#!/usr/bin/env python3
"""
Apply canonical (locked) assessments to all existing docs_v2 files.

Walks `docs_v2/gcp/**/*.json` and overwrites the canonical cross-cutting keys
(data-residency `location`/`region`/`zone`, and IAM common keys) with the single
source-of-truth values from `scripts/docgen_v3/lib/canonical.py`. New resources get the
...
def resource_name_from_path(path: Path) -> str:
    return path.name[:-5]  # strip .json; docs_v2 filenames are the full google_* type
