"""
Map each resource to its **verbatim** service/namespace (the docs_v2 folder name).

The provider schema has no notion of service grouping — that only lives in the
``subcategory`` field of each resource's markdown front matter. So we reuse docgen_v2's
...
``Cloud Storage`` → folder ``docs_v2/gcp/Cloud Storage/``.
"""
...
from scripts.docgen_v2.lib.parser import extract_subcategory_from_frontmatter
