# docgen_v3 — GCP resource docs generator (writes to `docs_v2/`)
...
`docgen_v2` with a corrected output contract; it writes to `docs_v2/` and leaves the
existing `docs/` tree untouched.

> Status: **GCP only.** AWS/Azure use different prefixes/grouping and are deferred.

## What's different from docgen_v2
...
scripts/docgen_v3/
  ...
  lib/
    ...
    service_map.py        # resource -> verbatim subcategory (reuses docgen_v2 clone)
...
Reuses from `scripts/docgen_v2/lib/`: `repository_manager` (sparse clone + version
detection), `parser.extract_subcategory_from_frontmatter`, `errors`, `logging_config`.

## Tests

```bash
uv run --extra dev pytest tests/docgen_v3/ -q
```
